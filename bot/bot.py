...
async def start_handle(update: Update, context: CallbackContext):
    ...
    reply_text = "Привет! Я бот <b>ChatGPT</b>, реализованный с помощью GPT-3.5 OpenAI API 🤖\n\n"
    ...
    async with user_semaphores[user_id]:
        ...
        if use_new_dialog_timeout:
            if (datetime.now() - db.get_user_attribute(user_id, "last_interaction")).seconds > config.new_dialog_timeout and len(db.get_dialog_messages(user_id)) > 0:
                db.start_new_dialog(user_id)
                await update.message.reply_text(f"Запуск нового диалога из-за режима ожидания (<b>{openai_utils.CHAT_MODES[chat_mode]['name']}</b> режима) ✅", parse_mode=ParseMode.HTML)
        db.set_user_attribute(user_id, "last_interaction", datetime.now())
...
async def show_balance_handle(update: Update, context: CallbackContext):
    ...
    text += f"Вы использовали <b>{n_used_tokens}</b> токены\n\n"

    text += "🏷️ Цены\n"
    text += f"<i>- ChatGPT: {price_per_1000_tokens}$ за 1000 токены\n"
    text += f"- Шепот (распознавание голоса): {config.whisper_price_per_1_min}$ за 1 минуту</i>"
